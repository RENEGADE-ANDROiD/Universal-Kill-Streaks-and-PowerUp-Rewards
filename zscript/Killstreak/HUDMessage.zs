Class PandHUDMessage : HUDMessageBase
{
	...
	static clearscope Pand_QueuedMsg Create(int targetplayer, Name fnt, string txt, Vector2 pos, Vector2 vsize = (0,0), int talign = -1, Vector2 balign = (0,0), int color = Font.CR_UNTRANSLATED, Vector3 time = (0,0,0), int wrap = 0, uint id = 0, int layer = BaseStatusBar.HUDMSGLayer_OverHUD )
	{
		let msg = new("Pand_QueuedMsg");
...
Class PandHUDMessageWaggle : PandHUDMessage
{
	...
	static clearscope Pand_QueuedMsgWaggle Create(int targetplayer, Name fnt, string txt, Vector2 pos, Vector2 vsize = (0,0), int talign = -1, Vector2 balign = (0,0), int color = Font.CR_UNTRANSLATED, Vector3 time = (0,0,0), double speed = 1.0, double strength = 1.0, double frequency = 1.0, int wrap = 0, uint id = 0, int layer = BaseStatusBar.HUDMSGLayer_OverHUD )
	{
		let msg = new("Pand_QueuedMsgWaggle");
		msg.targetplayer = targetplayer;
		msg.fnt = fnt;
		msg.txt = txt;
		msg.pos = pos;
...
Class Pand_QueuedMsgWaggle : Pand_QueuedMsg
{
	double speed, strength, frequency;

	override void AddSelf()
	{
		let msg = new("PandHUDMessageWaggle");
		msg.targetplayer = targetplayer;
		msg.fnt = fnt;
		msg.txt = txt;
		msg.pos = pos;
...
Class PandHUDMessageHandler : EventHandler
{
	...
	override void NetworkProcess( ConsoleEvent e )
	{
		if ( e.Name == 'TestMsg' )
		{
			...
			WaggleMsg(consoleplayer,'smallfont',"How did this get here I am not good with computer",(160,180),(320,200),time:(1,3,1), frequency:60.0);
			PlainMsg(consoleplayer,'STFOUCH0',"A A A",(80,50),(160,100),time:(1,3,1));
		}
	}
